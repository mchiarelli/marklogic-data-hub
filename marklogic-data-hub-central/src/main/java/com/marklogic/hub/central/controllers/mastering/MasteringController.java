...
public class MasteringController extends BaseController {
	...
	private Mastering newMasteringDSService() {
        return Mastering.on(getHubClient().getFinalClient());
    }
...
/*
	@RequestMapping(value = "/history", method = RequestMethod.POST)
	public  ResponseEntity<JsonNode> getHistory(HttpServletRequest request) throws IOException {

		JsonNode node = om.readTree(request.getInputStream());
		String uri = node.get("uri").asText();
		return ResponseEntity.ok(newMasteringDSService().getHistory(uri));
	}
*/	
